...
const AddProgressModal = forwardRef<AddProgressModalRef, AddProgressModalProps>(({ onSuccess: afterSuccess, onFail: afterFail }, ref) => {
  ...
  const notificationConfig = useMemo(() => {
    const baseMessage = `${baseToken?.amountToUse || 0} ${baseToken?.symbol} + ${quoteToken?.amountToUse || 0} ${quoteToken?.symbol}`
    return {
      loading: {
  ...
  const handleTransactions: AddProgressModalRef['showModal'] = async (baseToken, quoteToken, pool) => {
    setNextState()
  ...
  const showModal: AddProgressModalRef['showModal'] = (baseToken, quoteToken, pool) => {
    setOpen(true)
